...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp => grades.Where(g => emp.Sal >= g.Losal && emp.Sal <= g.Hisal )
                                            .Select(g => new {emp.EName, g.Grade})).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(m => m.DeptNo == e.DeptNo).Average(m => m.Sal)).Select(e => e.EName).ToList();
